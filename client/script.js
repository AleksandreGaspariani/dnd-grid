...
socket.on("chatMessage", (msg) => {
  ...
  if (userFigure) {
    ...
    setTimeout(
      () => bubble.parentElement && userFigure.removeChild(bubble),
      4000
    );
  }
});
    ...
    for (let x = 0; x < width; x++) {
      ...
      square.addEventListener("drop", (e) => {
        ...
        if (
          ...
          clearHighlights();
        } else if (data === "move" && isValidMove(targetX, targetY)) {
...
document.getElementById("userSetupForm").addEventListener("submit", (e) => {
  ...
  const selectedEmoji = document.querySelector(".emoji-btn.selected")?.dataset
    .emoji;

  if (!userName) return alert("Please enter a name");
  if (!selectedEmoji && imageInput.files.length === 0)
    return alert("Please pick an emoji or upload an image");

  if (imageInput.files.length > 0) {
    const file = imageInput.files[0];
    const reader = new FileReader();
    reader.onload = (event) =>
      socket.emit("pickUser", { name: userName, image: event.target.result });
...
socket.on("emojiList", (buffer) => {
  ...
  emojiButtons.forEach(
    (button) => (button.disabled = !emojis.includes(button.dataset.emoji))
  );
});
...
const highlightAvailableMoves = throttle((x, y, users) => {
  ...
  document.querySelectorAll(".grid-square").forEach((square) => {
    ...
    const isOccupied = Object.values(users).some(
      (u) => u.x === targetX && u.y === targetY && u !== users[socket.id]
    );
...
const clearHighlights = () => {
  document
    .querySelectorAll(".grid-square.highlight")
...
["moveUp", "moveDown", "moveLeft", "moveRight"].forEach((id, i) => {
  document
    ...
    .addEventListener("click", () =>
      moveFigure([0, 0, -1, 1][i], [-1, 1, 0, 0][i])
    );
});
...
socket.on("gridSizeUpdate", (buffer) =>
  drawGrid(...Object.values(decompressData(buffer)))
);
...
document
  .getElementById("backgroundImageUrlInput")
...
function dm() {
  ...
  if (isGuest) {
    // Enable guest DM mode
    document.querySelectorAll(".figure").forEach((figure) => {
      figure.draggable = true; // Enable drag-and-drop for all figures
    });
  }
}
...
const setSelectedUser = (userId) => {
  ...
  document.querySelectorAll(".figure").forEach((figure) => {
    ...
    figure.style.border = ""; // Reset border
  });
  const selectedFigure = document.querySelector(`.figure[data-id="${userId}"]`);
  if (selectedFigure) {
    selectedFigure.classList.add("selected");
    selectedFigure.style.border = "2px solid red"; // Add visible frame
  }
};

document.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("figure") &&
    (isGuest || !$("#dmControllers").hasClass("hidden"))
  ) {
    const userId = e.target.dataset.id;
    if (userId !== socket.id || isGuest) {
      // Guests can select any figure, players only in DM mode
...
document
  .getElementById("dmMoveUp")
  .addEventListener("click", () => dmMoveFigure(selectedUserId, 0, -1));
document
  .getElementById("dmMoveDown")
  .addEventListener("click", () => dmMoveFigure(selectedUserId, 0, 1));
document
  .getElementById("dmMoveLeft")
  .addEventListener("click", () => dmMoveFigure(selectedUserId, -1, 0));
document
  .getElementById("dmMoveRight")
  .addEventListener("click", () => dmMoveFigure(selectedUserId, 1, 0));

// Add key listeners for DM movements (W, A, S, D)
document.addEventListener("keydown", (e) => {
  if (!selectedUserId || (isGuest && $("#dmControllers").hasClass("hidden")))
    return; // Only work in DM mode for guests
